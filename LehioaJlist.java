...
public class LehioaJlist extends JFrame implements ActionListener{
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					LehioaJlist frame = new LehioaJlist();
	...
	public LehioaJlist() {
		...
		lblTexto = new JLabel("Aukeratu taldeak ...");
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (source == this.btnCopiarDerecha){
			...
			for(int posicion=0;posicion<numeroOpciones;posicion++){
				if (dlmCopia.contains(dlm.getElementAt(posicion))){
					JOptionPane.showMessageDialog(null, dlm.getElementAt(indices[posicion])+" aukeratuta duzu");
			...
			//this.dlmCopia.remove(lstGruposCopia.getSelectedIndex());
